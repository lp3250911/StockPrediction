    ...
    for index, row_concept in concept_stocks_df.iterrows():
        ...
        if ak.stock_board_concept_cons_em(symbol=row_concept[1]).size > 0:
            ...
            for index1, row_chengfengu in stock_board_concept_cons_em_df.iterrows():
                stock_zh_a_hist_df = ak.stock_zh_a_hist(symbol=row_chengfengu[1], period="daily", start_date="20210101",
                                                        end_date='20250101', adjust="")
...
def conceptbk():
    concept_stocks_df = ak.stock_board_concept_name_em()
    for index, row_concept in concept_stocks_df.iterrows():
        oard_concept_hist_em_df = ak.stock_board_concept_hist_em(symbol=row_concept[1], start_date="20240101",
                                                                 end_date="20250108", adjust="")
...
def sel_stocks():  # 单独导入日线数据
    stock_zh_a_hist_df = ak.stock_zh_a_hist(symbol='603259', period="daily", start_date="20210101", end_date='20250101',
                                            adjust="")
...
def append_stocks():
    ...
    rows = cur1.fetchall()
    column_names = ['code']
    del_df = pd.DataFrame(columns=column_names)
        ...
        if stockname.isdigit():
            ...
            for lt in lasttime:
                if len(lt) == 16:
                    ...
                    dochaxun = 0
                    new_recorde = pd.DataFrame([{"code": stockname}], index=[del_df.shape[0]])
                    pd.io.sql.to_sql(new_recorde, "sel_stocks", conn0, if_exists='append')
            if dochaxun == 1:
                date_str = last_day.strftime("%Y-%m-%d")
                date_obj = datetime.strptime(date_str, "%Y-%m-%d")
                next_day = date_obj + timedelta(days=1)
                str_day = next_day.strftime("%Y%m%d")
                stock_zh_a_hist_df = ak.stock_zh_a_hist(symbol=code, period="daily", start_date=str_day,
                                                        end_date='20250101', adjust="")
...
def append_conceptbk():
    ...
    for index, row_concept in concept_stocks_df.iterrows():
        ...
        next_day = date_obj + timedelta(days=1)
        str_day = next_day.strftime("%Y%m%d")
        oard_concept_hist_em_df = ak.stock_board_concept_hist_em(symbol=row_concept[1], start_date=str_day,
                                                                 end_date="20250108", adjust="")
...
def getallstocks():
    ...
    for index, row in filtered_stocks.iterrows():
        code_str = row["代码"][2:]
        name_str = row["名称"]
        stock_zh_a_hist_df = ak.stock_zh_a_hist(symbol=code_str, period="daily", start_date="20210101",
                                                end_date='20250530', adjust="")
